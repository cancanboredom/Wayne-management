...
%GIT% commit -m "%msg%"
echo.
echo Running changelog policy check before push...
node scripts/enforce-changelog-on-push.mjs
if errorlevel 1 (
    echo.
    echo [BLOCKED] Push cancelled by changelog policy.
    echo Please update CHANGELOG.md with correct version + layout, then try again.
    goto END
)
